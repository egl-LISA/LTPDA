...
function varargout = tohtml(varargin)
  ...
  elseif nargout == 0;
    ...
    htmltxt = [htmltxt sprintf('<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN"\n')];
    htmltxt = [htmltxt sprintf('   "http://www.w3.org/TR/1999/REC-html401-19991224/loose.dtd">\n\n')];
    
    htmltxt = [htmltxt sprintf('<html lang="en">\n')];
    
    % Header definition
    htmltxt = [htmltxt sprintf('  <head>\n')];
    htmltxt = [htmltxt sprintf('    <title>Method Info Report: %s/%s</title>\n', objs.mclass, objs.mname)];
    htmltxt = [htmltxt sprintf('    <link rel="stylesheet" type="text/css" href="%s">\n', docStyleFile)];
    htmltxt = [htmltxt sprintf('  </head>\n\n')];
    
    htmltxt = [htmltxt sprintf('  <body>\n\n')];
    
    htmltxt = [htmltxt txts]; % table
    
    htmltxt = [htmltxt sprintf('  </body>\n')];
    htmltxt = [htmltxt sprintf('</html>')];
    web(htmltxt);
  end
end
