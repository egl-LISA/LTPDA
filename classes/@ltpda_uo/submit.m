...
% for backwards compatibility convert any user supplied sinfo-structure
% into a plist
...
try
    ...
    for kk = 1:numel(objs)
        ...
        utils.helper.msg(msg.PROC1, 'submitting object: %s / %s', ...
            class(obj), obj.name);
        ...
        elseif isa(obj, 'ltpda_uo')
            ...
            error('Please define the created time for the following object [%s]', ...
                class(obj));
        end
...
subpl.pset('EXPERIMENT TITLE', sinfo.experiment_title);
subpl.pset('EXPERIMENT DESCRIPTION', sinfo.experiment_description);
subpl.pset('ANALYSIS DESCRIPTION', sinfo.analysis_description);
subpl.pset('QUANTITY', sinfo.quantity);
subpl.pset('KEYWORDS', sinfo.keywords);
subpl.pset('REFERENCE IDS', sinfo.reference_ids);
subpl.pset('ADDITIONAL COMMENTS', sinfo.additional_comments);
subpl.pset('ADDITIONAL AUTHORS', sinfo.additional_authors);
...
while iscell(objs) && isscalar(objs)
    objs = objs{1};
end
