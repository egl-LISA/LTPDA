...
function varargout = flscov(varargin)
  ...
  for iter = 1:maxiter
    ...
    [residuals, res_psd, w, model] = computeResiduals(in_aos, p, plk1, pl);
  ...
  if pl.find_core('doplot')
    try           
      ...
      % make the psd of the model
      model_ao_psd = psd(model, psdpl);    
      
      % select the frequency bin of the model
      model_ao_psd_select = select(model_ao_psd,plk_plot);
      
      % select the frequency range of the analysis for the analysis
      model_ao_psd_select_split = split(model_ao_psd_select, fpl);
      
      % plot psd of target versus residuals
      iplotPSD(target_ao_psd_split, res_psd, model_ao_psd_select_split, plist('errorbartype','area'));
  ...
  out_obj = collection(out_pest, residuals, model);
...
% Compute the residuals
%--------------------------------------------------------------------------
function [residual, res_psd, w, model] = computeResiduals(ts, p, plk1, pl)
  ...
  try
    ...
    try
      ...
      [model,ts(1)] = consolidate(model,ts(1),plist('fs',ts(1).fs));
      residual = ts(1) - model;
    end
