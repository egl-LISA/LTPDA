...
function varargout = resampleToCommonGrid(varargin)
  ...
  for kk = 1:numel(tsao)
    ...
    if ~isequal(tt, ts)
      ...
      interp_method = pl.find('interpolation method');
      tstr =  sprintf('%.17g + tsdata.createTimeVector(%.17f, %.17f)', tt(1), targetFs, nsecs);
      tsao(kk) = interp(tsao(kk), plist('vertices', tstr, 'method',interp_method));
...
function pl = buildplist()
  ...
  % Should truncate
  p = param({'interpolation method', 'The interpolation method'}, {'auto'});
  pl.append(p);
